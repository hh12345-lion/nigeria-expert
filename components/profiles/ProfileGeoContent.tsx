    ...
    <div className="my-8 space-y-8">
      {blocks.map((block) => {
          ...
          return (
            <ResponsiveTableWrap key={block.caption}>
              <table className="w-full min-w-[600px] border-collapse text-sm">
                <caption className="mb-3 text-left text-base font-semibold text-[#0D3B2E]">{block.caption}</caption>
                <thead>
                  <tr className="border-b border-[#D1E3D8] bg-[#F7F9F7]">
                    {block.headers.map((h) => (
                      <th key={h} className="px-4 py-3 text-left font-semibold text-[#0D3B2E]">
                        {h}
                      </th>
                    ))}
                  </tr>
        ...
        if (block.type === "list") {
          return (
            <div key={block.title}>
              <h2 className="text-xl font-bold text-[#0D3B2E]">{block.title}</h2>
